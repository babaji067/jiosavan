...
from ThakurMusic.misc import SUDOERS, db
...
from ThakurMusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from ThakurMusic.utils.inline.play import MORE_SETTINGS_UNTIL
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await ThakurMusic.pause_stream(chat_id)
        ...
        await ThakurMusic.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await ThakurMusic.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await ThakurMusic.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await ThakurMusic.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await ThakurMusic.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await ThakurMusic.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
...
async def markup_timer():
    while not await asyncio.sleep(7):
        ...
        for chat_id in active_chats:
            try:
                if MORE_SETTINGS_UNTIL.get(chat_id, 0) > time.monotonic():
                    continue
                if not await is_music_playing(chat_id):
                    continue
